...
router.post('/startgame', async (req, res) => {
    try {
        ...
        if(!StartGame(user, session.key, question)) {
            console.log(`Start Session Error :: Failed to establish socket room.`)
            res.json({ success: false, message: 'Failed to establish socket room.' })
            return
        }
...
router.post('/sendscore', async (req, res) => {
    try {
        ...
        const authKey = req.body.authKey
        const sessionKey = req.body.sessionKey
        const scoreUsers = req.body.scoreUsers
        ...
        const user = validUser.user
        SendScore(user, sessionKey, scoreUsers)
