...
export const SocketProvider = ({ children }) => {
    ...
    function Setup() {
        ...
        socket.on('user-join-game', (res) => {
            Notify(`${res.name} se unió al juego.`, 3000)
            GetConnectedUsers(res.key)
        })

        socket.on('user-leave-game', (res) => {
            Notify(`${res.name} dejó el juego.`, 3000)
            GetConnectedUsers(res.key)
        })

        if (host === null) {
            socket.on("move-to-game-page", () => {
                navigate('/questions');
            });
        }

        if (host !== null) {
            socket.on("get-userscore", (message) => {
                console.log('message', message)
                navigate('/winnerloser', { state: { message }} );
            });
        }
